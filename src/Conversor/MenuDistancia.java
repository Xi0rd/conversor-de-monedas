...
public class MenuDistancia extends JPanel {
	...
	public MenuDistancia() {
		...
		comboBoxDistancia.setModel(new DefaultComboBoxModel(new String[] {"Kilometros a Millas", "Metros a Yardas", "Metros a Pies ", "Centímetros a Pulgadas ", "Millas a Kilometros", "Yardas a Metros", "Pies a Metros", "Pulgadas a Centímetros"}));
		...
		btnOKdist.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (tipoCambio <= 3) {
					...
					JOptionPane.showMessageDialog(null, cantidadDist + " "+ArrayDistancia[tipoCambio].getDistancia1() +" equivalen a: "
							+ conversion + "  " + (ArrayDistancia[tipoCambio].getDistancia2()));
				} else {
					tipoCambio = tipoCambio - 4;
					BigDecimal divisor = new BigDecimal( ArrayDistancia[tipoCambio].getFactor());
					BigDecimal conversion= (cantidadDistBig.multiply(divisor).setScale(3, RoundingMode.HALF_UP));
					JOptionPane.showMessageDialog(null, cantidadDist + " " + (ArrayDistancia[tipoCambio].getDistancia2())
									+ " equivalen a: " + conversion + " "+(ArrayDistancia[tipoCambio].getDistancia1()));
				}
				int continuar= JOptionPane.showConfirmDialog(btnOKdist,"¿Desea continuar?", "CONTINUAR", JOptionPane.YES_NO_CANCEL_OPTION);
